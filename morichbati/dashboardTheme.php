...
$arrlength = count($Tname);
include 'Recommendation/recommTheme.php';
$sql = "SELECT * from theme WHERE Name IN ('$final_arr')";
$result = $mysqli->query($sql);
while ($row = $result->fetch_assoc()) {
    $Rname[] = $row["Name"];
    $Rphoto[] = $row["Photo"];
...
if (isset($_POST["themeSubmit"])) {
    $theme = $_POST["theme"];
    $sql = "UPDATE `applicant` SET `Theme Name`='" . $theme . "' where `ID` = '" . $id . "'";
    $result = $mysqli->query($sql);
    header("Refresh:0");
}
		...
		<div class="container">
			...
			<div class="col-sm-10">
				<div class="text-center">
					<?php if ($themeName != NULL){?>
						<h4>Your chosen theme: </h4><h2 style = "color: #f44b42; ">  <?php echo "$themeName"; ?></h2></br>
					<?php }?>	
					<h3>Themes recommended for you</h3>
				</div>
				<div class="row">
						...
						<div class="panel panel-default text-center">
							...
							<div class="panel-body">
								<a href="" data-toggle="modal" data-target="#platinum"><img
									src="themePic/<?php echo "$Rphoto[$x]"; ?>"
			...
			<?php } ?>
				...
				<p style="color: white; text-align: center">Choose Theme</p>
				<form name="form" action="" method="post">
					<div class="row">
						<div class="large-12 columns">


							<select name="theme" id="venue" style="width: 100%;">
							<?php for($x = 0; $x <$arrlength; $x++) {?>
								<option value="<?php echo "$Tname[$x]"; ?>"><?php echo "$Tname[$x]"; ?></option>
							<?php }?>	
							</select> <input type="submit" value="Submit"
								name="themeSubmit" class="btn btn-primary btn-block"
